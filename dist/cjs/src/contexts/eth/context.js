...
exports.EthContext = exports.INSUFFICIENT_ALLOWANCE = exports.NO_VAA_FOUND = void 0;
const ethers_contracts_1 = require("@certusone/wormhole-sdk/lib/esm/ethers-contracts");
...
exports.NO_VAA_FOUND = 'No message publish found in logs';
exports.INSUFFICIENT_ALLOWANCE = 'Insufficient token allowance';
class EthContext extends relayer_1.RelayerAbstract {
    constructor(context, foreignAssetCache) {
        super();
        this.type = types_1.Context.ETH;
        this.context = context;
        this.contracts = new contracts_1.EthContracts(context);
    ...
    async getForeignAssetPartiallyUnresolved(tokenId, chain, provider) {
        ...
        return async () => {
            const foreignAddr = await tokenBridge.wrappedAsset(chainId, ethers_2.utils.arrayify(tokenAddr));
            if (foreignAddr === ethers_1.constants.AddressZero)
                return null;
    ...
    async fetchTokenDecimals(tokenAddr, chain) {
        const provider = this.context.mustGetProvider(chain);
        const tokenContract = ethers_contracts_1.TokenImplementation__factory.connect(tokenAddr, provider);
    ...
    async getTokenBalance(walletAddr, tokenId, chain) {
        ...
        if (!address)
            return null;
        const provider = this.context.mustGetProvider(chain);
        const token = ethers_contracts_1.TokenImplementation__factory.connect(address, provider);
    ...
    async getTokenBalances(walletAddr, tokenIds, chain) {
        ...
        const provider = rpc.startsWith('http://') || rpc.startsWith('https://')
            ? new ethers_1.ethers.providers.JsonRpcBatchProvider(rpc)
            : contextProvider;
        ...
        let addresses = [];
        {
            ...
            const idxChunks = (0, utils_2.chunkArray)(unresolvedIndexes, maxBatchSize);
        ...
        let balances = [];
        {
            ...
            const idxChunks = (0, utils_2.chunkArray)(nonNullIndexes, maxBatchSize);
            let queriedBalances = [];
            // batch request each chunk
            for (const chunk of idxChunks) {
                queriedBalances = [
                    ...
                    ...(await Promise.all(chunk.map((idx) => {
                        ...
                        return !address
                            ...
                            : // TODO: this connect may trigger extra requests
                                ethers_contracts_1.TokenImplementation__factory.connect(address, provider).balanceOf(walletAddr);
                    }))),
                ];
            }
            // re-assemble the balances array to match the input order
    ...
    async approve(chain, contractAddress, token, amount, overrides = {}) {
        ...
        const tokenImplementation = ethers_contracts_1.TokenImplementation__factory.connect(token, signer);
        ...
        const approveAmount = amount || ethers_1.constants.MaxUint256;
        ...
        if (approved.lt(approveAmount)) {
            ...
            if (nowApproved.lt(approveAmount)) {
                throw new Error(exports.INSUFFICIENT_ALLOWANCE);
            }
        }
    }
    async prepareSend(token, amount, sendingChain, senderAddress, recipientChain, recipientAddress, relayerFee = 0, overrides = {}) {
        ...
        if (token === types_1.NATIVE) {
            // sending native ETH
            await bridge.callStatic.wrapAndTransferETH(recipientChainId, destContext.formatAddress(recipientAddress), relayerFee, (0, wormhole_sdk_1.createNonce)(), {
                ...overrides,
                value: amountBN,
                from: senderAddress,
            });
            return bridge.populateTransaction.wrapAndTransferETH(recipientChainId, destContext.formatAddress(recipientAddress), relayerFee, (0, wormhole_sdk_1.createNonce)(), {
                ...overrides,
                value: amountBN,
            });
        }
            ...
            await bridge.callStatic.transferTokens(tokenAddr, amountBN, recipientChainId, destContext.formatAddress(recipientAddress), relayerFee, (0, wormhole_sdk_1.createNonce)(), {
                ...overrides,
                from: senderAddress,
            });
            return bridge.populateTransaction.transferTokens(tokenAddr, amountBN, recipientChainId, destContext.formatAddress(recipientAddress), relayerFee, (0, wormhole_sdk_1.createNonce)(), overrides);
        }
    }
    ...
    async sendWithPayload(token, amount, sendingChain, senderAddress, recipientChain, recipientAddress, payload, overrides = {}) {
        ...
        if (token === types_1.NATIVE) {
            // sending native ETH
            const v = await bridge.wrapAndTransferETHWithPayload(recipientChainId, destContext.formatAddress(recipientAddress), (0, wormhole_sdk_1.createNonce)(), payload, {
                ...overrides,
                value: amountBN,
            });
            return await v.wait();
            ...
            const v = await bridge.transferTokensWithPayload(tokenAddr, amountBN, recipientChainId, destContext.formatAddress(recipientAddress), (0, wormhole_sdk_1.createNonce)(), payload, overrides);
            return await v.wait();
        }
    }
    async prepareSendWithRelay(token, amount, toNativeToken, sendingChain, senderAddress, recipientChain, recipientAddress, overrides = {}) {
        ...
        const nativeTokenBN = ethers_1.ethers.BigNumber.from(toNativeToken);
        if (token === types_1.NATIVE) {
            // sending native ETH
    ...
    async getMessage(tx, chain, parseRelayerPayload = true) {
        ...
        if (bridgeLogs.length === 0) {
            throw new Error(exports.NO_VAA_FOUND);
        }
        let gasFee = ethers_1.BigNumber.from(0);
        if (receipt.gasUsed && receipt.effectiveGasPrice) {
            gasFee = receipt.gasUsed.mul(receipt.effectiveGasPrice);
        }
        const fromChain = this.context.toChainName(chain);
        const parsed = ethers_contracts_1.Implementation__factory.createInterface().parseLog(bridgeLogs[0]);
        ...
        if (parsed.args.payload.startsWith('0x01')) {
            ...
            const tokenAddress = await tokenContext.parseAssetAddress(ethers_2.utils.hexlify(transfer.tokenAddress));
            return {
                sendTx: tx,
                sender: receipt.from,
                ...
                emitterAddress: ethers_2.utils.hexlify(this.formatAddress(bridge.address)),
        ...
        const destContext = this.context.getContext(toChain);
        const tokenContext = this.context.getContext(tokenChain);
        const tokenAddress = await tokenContext.parseAssetAddress(ethers_2.utils.hexlify(transferWithPayload.tokenAddress));
         ...
         */
        const relayerPayload = destContext.parseRelayerPayload(Buffer.from((0, utils_1.arrayify)(transferWithPayload.payload)));
        const relayerMessage = {
            ...
            to: destContext.parseAddress(ethers_2.utils.hexlify(transferWithPayload.to)),
            payload: transferWithPayload.payload,
        };
    ...
    async getRelayerFee(sourceChain, destChain, tokenId) {
        ...
        const tokenContract = ethers_contracts_1.TokenImplementation__factory.connect(address, provider);
        const decimals = await tokenContract.decimals();
    ...
    async isTransferCompleted(destChain, signedVaa) {
        ...
        const doubleHash = (0, wormhole_sdk_1.keccak256)((0, vaa_1.parseVaa)(ethers_2.utils.arrayify(signedVaa, { allowMissingPrefix: true })).hash);
        return await tokenBridge.isTransferCompleted(doubleHash);
    }
    formatAddress(address) {
        return Buffer.from(ethers_2.utils.zeroPad(address, 32));
    }
    parseAddress(address) {
        const parsed = ethers_2.utils.hexlify(ethers_2.utils.stripZeros(address));
        return ethers_2.utils.getAddress(parsed);
    }
