...
// Parse a signature
const signatureParser = new binary_parser_1.Parser()
    .endianess('big')
...
const vaaParser = new binary_parser_1.Parser()
    .endianess('big')
    .uint8('version')
...
function vaaDigest(vaa) {
    return (0, utils_1.solidityKeccak256)(['bytes'], [(0, utils_1.solidityKeccak256)(['bytes'], ['0x' + vaaBody(vaa)])]);
}
...
const addressParser = (length) => new binary_parser_1.Parser().endianess('big').array('address', {
    type: 'uint8',
    lengthInBytes: length,
    formatter: (arr) => Buffer.from(arr).toString('hex'),
});
// Parse a guardian set upgrade payload
const guardianSetUpgradeParser = new P(new binary_parser_1.Parser()
    .endianess('big')
    .string('module', {
    length: 32,
    encoding: 'hex',
...
const coreContractUpgradeParser = new P(new binary_parser_1.Parser()
    .endianess('big')
    .string('module', {
    length: 32,
    encoding: 'hex',
...
function tokenBridgeTransferParser() {
    return new P(new binary_parser_1.Parser()
        ...
        type: 'uint8',
        lengthInBytes: 32,
        formatter: (bytes) => ethers_1.BigNumber.from(bytes).toBigInt(),
    })
        .array('tokenAddress', {
        type: 'uint8',
        lengthInBytes: 32,
