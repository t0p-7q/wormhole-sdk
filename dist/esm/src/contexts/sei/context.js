import { CHAIN_ID_SEI, buildTokenId, cosmos, hexToUint8Array, isNativeCosmWasmDenom, parseTokenTransferPayload, parseVaa, } from '@certusone/wormhole-sdk';
import { CosmWasmClient } from '@cosmjs/cosmwasm-stargate';
import { decodeTxRaw } from '@cosmjs/proto-signing';
import { calculateFee, logs as cosmosLogs, } from '@cosmjs/stargate';
import axios from 'axios';
import base58 from 'bs58';
import { MsgExecuteContract } from 'cosmjs-types/cosmwasm/wasm/v1/tx';
import { BigNumber } from 'ethers';
import { arrayify, base64, hexStripZeros, hexlify, keccak256, zeroPad, } from 'ethers/lib/utils';
 ...
 */
export class SeiContext extends TokenBridgeAbstract {
    constructor(context, foreignAssetCache) {
        super();
        this.context = context;
        this.type = Context.SEI;
    ...
    async isTranslatedToken(tokenAddress) {
        ...
        const resp = await axios.get(`${new URL(this.context.conf.rest.sei)}sei-protocol/seichain/tokenfactory/denoms_from_creator/${this.getTranslatorAddress()}`);
    ...
    buildNativeId() {
        return ('01' + keccak256(Buffer.from(this.NATIVE_DENOM, 'utf-8')).substring(4));
    }
    /**
    ...
    async queryExternalId(externalId) {
        ...
        try {
            const response = await wasmClient.queryContractSmart(tokenBridgeAddress, {
                external_id: {
                    external_id: Buffer.from(stripHexPrefix(externalId), 'hex').toString('base64'),
                },
            });
    ...
    async getMessage(id, chain, parseRelayerPayload = true) {
        ...
        const parsed = parseTokenTransferPayload(Buffer.from(tokenTransferPayload, 'hex'));
    ...
    async getCosmWasmClient() {
        if (!this.wasmClient) {
            ...
            this.wasmClient = await CosmWasmClient.connect(this.context.conf.rpcs.sei);
        }
