 ...
 * @category Solana
 */
export async function postVaaWithRetry(connection, signTransaction, wormholeProgramId, payer, vaa, maxRetries, commitment) {
 ...
 * @category Solana
 */
export async function postVaa(connection, signTransaction, wormholeProgramId, payer, vaa, options, asyncVerifySignatures = true) {
    ...
    const postVaaTransaction = unsignedTransactions.pop();
    const verifySignatures = async (transaction) => signSendAndConfirmTransaction(connection, payer, modifySignTransaction(signTransaction, ...signers), transaction, options);
    const output = [];
    ...
    output.push(await signSendAndConfirmTransaction(connection, payer, signTransaction, postVaaTransaction, options));
    return output;
}
/**
 * @category Solana
 ...
 *
 */
export async function createPostSignedVaaTransactions(connection, wormholeProgramId, payer, vaa, commitment) {
    const parsed = isBytes(vaa) ? parseVaa(vaa) : vaa;
    const signatureSet = Keypair.generate();
    const verifySignaturesInstructions = await createVerifySignaturesInstructions(connection, wormholeProgramId, payer, parsed, signatureSet.publicKey, commitment);
    ...
    unsignedTransactions.push(new Transaction().add(createPostVaaInstruction(connection, wormholeProgramId, payer, parsed, signatureSet.publicKey)));
    return {
