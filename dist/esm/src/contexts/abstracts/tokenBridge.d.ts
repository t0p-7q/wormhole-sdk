 ...
 */
export declare abstract class TokenBridgeAbstract<TransactionResult> {
    /**
     ...
     */
    protected abstract send(token: TokenId | 'native', amount: string, sendingChain: ChainName | ChainId, senderAddress: string, recipientChain: ChainName | ChainId, recipientAddress: string, relayerFee: any): Promise<TransactionResult>;
    /**
     ...
     * @returns The transaction receipt
     */
    protected abstract sendWithPayload(token: TokenId | 'native', amount: string, sendingChain: ChainName | ChainId, senderAddress: string, recipientChain: ChainName | ChainId, recipientAddress: string, payload: any): Promise<TransactionResult>;
    protected abstract estimateSendGas(token: TokenId | typeof NATIVE, amount: string, sendingChain: ChainName | ChainId, senderAddress: string, recipientChain: ChainName | ChainId, recipientAddress: string): Promise<BigNumber>;
    protected abstract estimateClaimGas(destChain: ChainName | ChainId, VAA: Uint8Array): Promise<BigNumber>;
    /**
     ...
     */
    protected abstract getForeignAsset(tokenId: TokenId, chain: ChainName | ChainId): Promise<string | null>;
    /**
     * Fetches the address for a token representation on any chain (These are the Wormhole token addresses, not necessarily the cannonical version of that token)
     *
     * @param tokenId The Token ID (chain/address)
     ...
     */
    protected abstract mustGetForeignAsset(tokenId: TokenId, chain: ChainName | ChainId): Promise<string>;
    /**
     ...
     */
    protected abstract getNativeBalance(walletAddress: string, chain: ChainName | ChainId): Promise<BigNumber>;
    /**
     ...
     */
    protected abstract getTokenBalance(walletAddress: string, tokenId: TokenId, chain: ChainName | ChainId): Promise<BigNumber | null>;
    /**
     ...
     * @returns The token balance of the wormhole asset as a BigNumber
     */
    protected abstract getTokenBalances(walletAddress: string, tokenIds: TokenId[], chain: ChainName | ChainId): Promise<(BigNumber | null)[]>;
    protected abstract getTxGasFee(txId: string, chain: ChainName | ChainId): Promise<BigNumber | undefined>;
    /**
     ...
     * @returns The transaction receipt
     */
    protected abstract redeem(destChain: ChainName | ChainId, signedVAA: Uint8Array, overrides: any, payerAddr?: any): Promise<TransactionResult>;
    /**
     ...
     */
    protected abstract isTransferCompleted(destChain: ChainName | ChainId, signedVaa: string): Promise<boolean>;
}
